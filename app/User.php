...
class User extends Model {
    ...
    static public function verifyUser($request) {
        ...
        if ($user) {
            $user = $user[0];
            if (Hash::check($request['log_password'], trim($user->password))) {
                $valid = true;
                Session::put('user_id', trim($user->id));
                Session::put('user_name', trim( $user->name));
                if ($user->role == 3) {
                    Session::put('is_admin', true);
                }
                Session::flash('sm', 'welcome back, ' . '<b>' . trim($user->name) . '</b>');
            }
        }
        return $valid;
    }
